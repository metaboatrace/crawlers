...
def parse_args():  # type: ignore
    parser = argparse.ArgumentParser(
        description="Remove data not included in specified racer evaluation term."
    )
...
def delete_data_outside_term(session, term_start, term_end) -> None:  # type: ignore
    for table, date_column in TABLE_DATE_COLUMN_MAP.items():
        logger.info(f"Deleting data from {table} outside of {term_start} to {term_end}...")
        delete_query = text(
            f"DELETE FROM {table} WHERE {date_column} NOT BETWEEN :term_start AND :term_end"
        )
...
def main() -> None:
    ...
    confirmation = input(
        f"Remove data not included between {term_start} and {term_end}. Are you sure? (y/N): "
    )
